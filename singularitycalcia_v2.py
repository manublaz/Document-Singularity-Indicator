...
# -*- coding: utf-8 -*-
"""
singularitycalcia_v2.py  —  IS_d v2.0
======================================
Indicador de Singularidad Documental para selección de corpus en IA
...
Salidas generadas (en --output DIR):
  calcia_singularity_results.csv   -- tabla completa (todos los documentos)
  calcia_top50.csv                 -- top-50 por IS_d v2
  ...
  fig4_scatter_citas.png           -- IS_d v2 vs citas
...
def load_from_db(db_path: str, limit: Optional[int] = None) -> List[Dict]:
    ...
    print(f'  Columnas detectadas: {sorted(cols)}')
    ...
    print(f'  Documentos validos: {len(docs):,}  '
          f'(omitidos por texto corto: {skipped})')
    return docs
...
def db_summary(db_path: str) -> None:
    ...
    conn.close()
    print(f'\n  {"=" * 52}')
    print(f'  RESUMEN DE calcia.db')
    print(f'  {"=" * 52}')
    print(f'  Total registros       : {total:,}')
    print(f'  Con texto suficiente  : {with_text:,}')
    print(f'  Con citas > 0         : {with_cit:,}')
    if cmax:
        print(f'  Citas min/max/media   : {cmin} / {cmax} / {cavg:.1f}')
    print(f'  {"=" * 52}\n')
...
class SingularityV1:
    ...
    def build(self, documents: List[Dict]) -> None:
        ...
        for i, doc in enumerate(documents, 1):
            ...
            for ng in cntr:
                self.doc_count[ng] += 1
        print(f'  Indice listo en {time.time()-t0:.1f}s  '
              f'| n-gramas unicos: {len(self.corpus_ng):,}')
    ...
    def analyze(self, documents: List[Dict]) -> pd.DataFrame:
        ...
        print('  Calculando IS_d v1...')
        rows  = []
        n     = len(documents)
        t0    = time.time()
        for i, doc in enumerate(documents, 1):
            if i % 50 == 0 or i == n:
                _progress(i, n, 'IS_d v1', t_start=t0)
        ...
        print(f'  IS_d v1 completado en {elapsed:.1f}s  '
...
class SingularityV2:
    ...
    def build(self, documents: List[Dict]) -> None:
        print('  Construyendo indice IS_d v2...')
        ...
        print(f'    Corpus: {self.C:,} docs  |  '
              f'n-gramas unicos: {len(self.corpus_ng):,}  |  '
              f'instancias totales: {sum(self.corpus_ng.values()):,}')
    ...
    def analyze(self, documents: List[Dict]) -> pd.DataFrame:
        ...
        print(f'    P99(O_d_raw) = {p99_O:.6f}  | tiempo: {time.time()-t0:.1f}s')

        # Paso 2: scores finales
        print('  Calculando IS_d v2 (2/2)...')
        rows = []
        t0   = time.time()
        for i, (doc, od_raw) in enumerate(zip(documents, raw), 1):
            if i % 100 == 0 or i == n:
                _progress(i, n, 'IS_d v2', t_start=t0)
        ...
        print(f'  IS_d v2 completado en {time.time()-t0:.1f}s')
        return pd.DataFrame(rows)
...
def analyze_synthetic(n: int = 3, alpha: float = 0.7,
    ...
    print('\n' + '-'*60)
    print('VALIDACION -- CORPUS SINTETICO (ground truth conocido)')
    print('-'*60)
    ...
    print('\n  Resultados:')
    print(f"  {'ID':<6} {'Cat':>3} {'Citas':>7} {'v1':>9} {'v2':>9} "
          f"{'O_d':>7} {'I_d':>7}  Titulo")
    print('  ' + '-'*80)
...
def compute_correlations(df: pd.DataFrame, score_col: str) -> Dict:
    ...
    corr = {}
    mapping = {
        'log(1+citas)': np.log1p(df['citations'].clip(0)),
        'Citas brutas': df['citations'],
    }
    if 'text_length' in df.columns:
        mapping['Longitud texto']   = df['text_length']
    if 'N_total' in df.columns:
        mapping['N-gramas unicos (v1)'] = df['N_total']
    if 'N_total_v2' in df.columns:
        mapping['N-gramas unicos (v2)'] = df['N_total_v2']
...
def plot_histogramas(df, out):
    ...
    fig.suptitle('Figura 1 -- Distribucion del score IS_d: v1 vs v2',
                 fontsize=11, fontweight='bold', color=COLORS['deep'])
    bins = np.linspace(0, 1, 45)
    for ax, col, color, label in [
        (axes[0], 'score_v1', COLORS['v1'], 'IS_d v1 (original)'),
        (axes[1], 'score_v2', COLORS['v2'], 'IS_d v2 (mejorado)'),
    ]:
        s = df[col]
        ...
        ax.axvline(s.mean(),   color=COLORS['accent'], lw=1.5, ls=':',
                   label=f'Media={s.mean():.3f}')
        _ax(ax, label, 'Score IS_d', 'Frecuencia')
...
def plot_ceros_y_iqr(df, out):
    ...
    fig.patch.set_facecolor('white')
    fig.suptitle('Figura 2 -- Metricas de calidad discriminatoria: IS_d v1 vs IS_d v2',
                 fontsize=11, fontweight='bold', color=COLORS['deep'])
    ...
    for ax, (titulo, vals) in zip(axes, metrics):
        bars = ax.bar(['IS_d v1','IS_d v2'], vals,
...
def plot_correlaciones(df, out):
    variables = {'log(1+citas)': np.log1p(df['citations'].clip(0))}
    if 'text_length' in df.columns:
        variables['Longitud texto'] = df['text_length']
    col_n = 'N_total_v2' if 'N_total_v2' in df.columns else 'N_total'
    if col_n in df.columns:
        variables['N-gramas unicos'] = df[col_n]
    ...
    fig.patch.set_facecolor('white')
    b1 = ax.bar(x-w/2, v1c, w, label='IS_d v1',
                color=COLORS['v1'], alpha=0.9, edgecolor='white')
    b2 = ax.bar(x+w/2, v2c, w, label='IS_d v2',
    ...
    _ax(ax, 'Figura 3 -- Correlaciones de Pearson (IS_d vs variables del corpus)',
        ylabel='r de Pearson')
...
def plot_scatter_citas(df, out):
    ...
    fig.patch.set_facecolor('white')
    fig.suptitle('Figura 4 -- IS_d v2 vs Impacto Bibliometrico',
                 fontsize=11, fontweight='bold', color=COLORS['deep'])
    ...
    r = float(y.corr(x))
    _ax(ax, f'Densidad hexagonal  [r = {r:.3f}]', 'log(1+citas)', 'IS_d v2')
    ...
    try:
        ...
        _ax(ax, 'IS_d v2 por cuartil de citas', 'Cuartil de citas', 'IS_d v2')
...
def plot_boxplot_cuartiles(df, out):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    fig.patch.set_facecolor('white')
    fig.suptitle('Figura 5 -- Distribucion de scores por cuartil de citas',
                 fontsize=11, fontweight='bold', color=COLORS['deep'])
    for ax, col, color, label in [
        (axes[0], 'score_v1', COLORS['v1'], 'IS_d v1'),
        (axes[1], 'score_v2', COLORS['v2'], 'IS_d v2'),
    ]:
        ...
        _ax(ax, label, 'Cuartil de citas', 'Score')
    plt.tight_layout()
    _save(fig, out)
...
def plot_componentes(df, out):
    ...
    fig, axes = plt.subplots(1, 3, figsize=(13, 4.5))
    fig.patch.set_facecolor('white')
    fig.suptitle('Figura 6 -- Descomposicion IS_d v2: O_d (Originalidad) vs I_d (Impacto)',
    ...
    plt.colorbar(sc, ax=ax, label='IS_d v2')
    _ax(ax, 'O_d vs I_d', 'I_d (Impacto)', 'O_d (Originalidad)')
    for ax2, col, color, tit in [
        (axes[1], 'Od', COLORS['v2'], 'Distribucion O_d'),
        (axes[2], 'Id', COLORS['v1'], 'Distribucion I_d'),
    ]:
        ax2.hist(df[col], bins=40, color=color, alpha=0.85,
                 edgecolor='white', lw=0.3)
        ax2.axvline(df[col].median(), color=COLORS['deep'], lw=1.5, ls='--')
        _ax(ax2, tit, col, 'Frecuencia')
    plt.tight_layout()
    _save(fig, out)
...
def plot_alpha_sensitivity(df, out):
    ...
    fig.patch.set_facecolor('white')
    fig.suptitle('Figura 7 -- Sensibilidad al parametro alpha (peso de originalidad)',
                 fontsize=11, fontweight='bold', color=COLORS['deep'])
    for ax, col, ylabel, color in [
        (axes[0], 'corr_cit',  'r con log(citas)',   COLORS['v2']),
        (axes[1], 'iqr',       'IQR del score',      COLORS['accent']),
        (axes[2], 'corr_text', 'r con longitud txt', COLORS['v1']),
    ]:
...
def plot_top_bottom(df, score_col, n, out):
    ...
    fig.patch.set_facecolor('white')
    fig.suptitle(f'Figura 8 -- Top {n} y Bottom {n} documentos por {score_col}',
                 fontsize=11, fontweight='bold', color=COLORS['deep'])
...
def plot_deciles(dec_df, out):
    ...
    fig, axes = plt.subplots(1, 3, figsize=(13, 4.5))
    fig.patch.set_facecolor('white')
    fig.suptitle('Figura 9 -- Perfil de caracteristicas por decil de IS_d v2',
                 fontsize=11, fontweight='bold', color=COLORS['deep'])
    pal = plt.cm.RdYlGn(np.linspace(0.15, 0.9, len(dec_df)))
    decile_labels = dec_df['decile'].astype(str).tolist()
    for ax, col, label in [
        (axes[0], 'citas_median',  'Citas medianas'),
        (axes[1], 'text_len_mean', 'Long. texto media (chars)'),
        (axes[2], 'ngrams_uniq',   'N-gramas unicos medios'),
    ]:
        ...
        _ax(ax, label, 'Decil IS_d v2', label)
...
def _plot_synthetic(df, out):
    cats      = ['A','B','C','D']
    cat_lbls  = {'A':'A: Fundacional','B':'B: Medio',
                 'C':'C: Bajo impacto','D':'D: Abstracts cortos'}
    pal       = {'A':COLORS['green'],'B':COLORS['mid'],
                 'C':COLORS['v2'],   'D':COLORS['accent']}
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    fig.patch.set_facecolor('white')
    fig.suptitle('Figura 10 -- Validacion corpus sintetico (ground truth conocido)',
                 fontsize=11, fontweight='bold', color=COLORS['deep'])
    ax  = axes[0]
    ...
    ax.bar(x-w/2, v1m, w, label='IS_d v1', color=COLORS['v1'],
           alpha=0.85, edgecolor='white')
    ax.bar(x+w/2, v2m, w, label='IS_d v2', color=COLORS['v2'],
           alpha=0.85, edgecolor='white')
    ax.set_xticks(x); ax.set_xticklabels(cats)
    ax.set_ylim(0, 1.15)
    _ax(ax, 'Score medio por categoria', 'Categoria', 'Score IS_d')
    ax.legend(fontsize=9)
    ax = axes[1]
    for cat in cats:
    ...
    _ax(ax, 'IS_d v2 vs log(1+citas)', 'log(1+citas)', 'IS_d v2')
    ax.legend(fontsize=8)
    plt.tight_layout()
    _save(fig, out)
...
def write_report(df, stats_v1, stats_v2, corr_v1, corr_v2,
    ...
    lines += [SEP,
              '  IS_d v2 -- REPORTE DE ANALISIS COMPLETO',
              f'  Parametros: n-grama={n}  alpha={alpha}  beta={DEFAULT_BETA}',
              f'  Generado: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}',
              SEP]

    # Estadisticas comparativas
    sec('ESTADISTICAS DESCRIPTIVAS -- IS_d v1 vs IS_d v2')
    lines.append(f"  {'Metrica':<28} {'IS_d v1':>12} {'IS_d v2':>12}  {'Cambio':>10}")
    lines.append('  ' + '-'*65)
    fields = [
        ('n_total',   'N documentos',     False),
        ('n_zero',    'Docs score = 0',   True),
        ('pct_zero',  '% score = 0',      True),
        ('mean',      'Media',            False),
        ('std',       'Desv. tipica',     False),
        ('min',       'Minimo',           False),
        ('q1',        'Q1 (P25)',         False),
        ('median',    'Mediana (P50)',     False),
        ('q3',        'Q3 (P75)',         False),
        ('p90',       'P90',              False),
        ('max',       'Maximo',           False),
        ('iqr',       'IQR',              True),
        ('skewness',  'Asimetria',        False),
        ('kurtosis',  'Curtosis',         False),
    ]
    ...
    # Correlaciones
    sec('CORRELACIONES DE PEARSON')
    lines.append(f"  {'Variable':<30} {'IS_d v1':>10} {'IS_d v2':>10}  {'Mejora':>8}")
    lines.append('  ' + '-'*62)
    ...
    if dec_df is not None and not dec_df.empty:
        sec('TABLA POR DECILES DE IS_d v2 (documentos con score > 0)')
        hdr = ['Decil','N','Score media','Citas median','Long. texto','N-grams unicos']
    ...
    # Top 25
    sec('TOP 25 DOCUMENTOS POR IS_d v2')
    lines.append(f"  {'#':<4} {'IS_d v2':>8} {'IS_d v1':>8} "
                 f"{'O_d':>7} {'I_d':>7} {'Citas':>8}  Titulo")
    lines.append('  ' + '-'*100)
    ...
    # Bottom 25
    sec('BOTTOM 25 DOCUMENTOS POR IS_d v2 (excl. score=0)')
    lines.append(f"  {'#':<4} {'IS_d v2':>8} {'IS_d v1':>8} "
                 f"{'O_d':>7} {'I_d':>7} {'Citas':>8}  Titulo")
    lines.append('  ' + '-'*100)
...
def run_db_analysis(db_path, alpha, ngram, output_dir, limit=None):
    ...
    print(f'\n{"="*60}')
    print(f'  IS_d v2  --  ANALISIS SOBRE calcia.db')
    print(f'  alpha={alpha}  n-grama={ngram}  output={output_dir}')
    print(f'{"="*60}')

    db_summary(db_path)
    documents = load_from_db(db_path, limit)
    if not documents:
        print('  ERROR: Sin documentos. Abortando.')
        return
    ...
    print(f'    Tiempo: {(datetime.now()-t0).seconds}s')
    ...
    stats_v1 = compute_stats(df['score_v1'], 'IS_d v1')
    stats_v2 = compute_stats(df['score_v2'], 'IS_d v2')
    ...
    # Resumen en consola
    print(f'\n  {"="*50}')
    print(f'  {"Metrica":<28} {"IS_d v1":>10} {"IS_d v2":>10}')
    print(f'  {"-"*50}')
    for key, label in [
        ('n_total','N docs'), ('pct_zero','% score=0'),
        ('median','Mediana'), ('iqr','IQR'), ('skewness','Asimetria')]:
        print(f'  {label:<28} {stats_v1[key]:>10.3f} {stats_v2[key]:>10.3f}')
    print(f'  {"-"*50}')
    print(f'  Correlacion con log(citas):')
    print(f'    v1: {corr_v1.get("log(1+citas)",0):>7.4f}')
    print(f'    v2: {corr_v2.get("log(1+citas)",0):>7.4f}')
    print(f'  {"="*50}')
    ...
    stats_rows = []
    for key, label in [
        ('n_total','N documentos'), ('n_zero','Docs score=0'),
        ('pct_zero','% score=0'), ('mean','Media'), ('std','Desv. tipica'),
        ('min','Minimo'), ('q1','Q1'), ('median','Mediana'),
        ('q3','Q3'), ('p90','P90'), ('max','Maximo'),
        ('iqr','IQR'), ('skewness','Asimetria'), ('kurtosis','Curtosis')]:
    ...
    print(f'\n{"="*60}')
    print(f'  ANALISIS COMPLETO. Archivos en: {output_dir}/')
    print(f'{"="*60}\n')
...
def run_csv_analysis(csv_path, alpha, ngram, output_dir):
    ...
    if 'score_v1' not in df.columns:
        print('  ERROR: Columna score_v1 no encontrada.')
        return
    ...
    write_report(df, stats_v1, stats_v2, corr_v1, corr_v2,
                 dec_df, alpha, ngram, p('singularity_report.txt'))
    print(f'\n  ANALISIS CSV completo. Archivos en: {output_dir}/')
...
def main():
    parser = argparse.ArgumentParser(
        description='IS_d v2 -- Indicador de Singularidad Documental para IA',
...
""")
    parser.add_argument('--db',        type=str,
                        help='Ruta a calcia.db')
    parser.add_argument('--csv',       type=str,
                        help='Ruta a CSV de resultados v1 previos')
    parser.add_argument('--synthetic', action='store_true',
                        help='Ejecutar solo validacion con corpus sintetico')
    parser.add_argument('--alpha',     type=float, default=DEFAULT_ALPHA,
                        help=f'Peso de originalidad alpha [0-1]  (defecto: {DEFAULT_ALPHA})')
    parser.add_argument('--ngram',     type=int,   default=DEFAULT_NGRAM,
                        help=f'Tamano de n-grama  (defecto: {DEFAULT_NGRAM})')
    parser.add_argument('--limit',     type=int,   default=None,
                        help='Limitar num. documentos (para pruebas rapidas)')
    parser.add_argument('--output',    type=str,   default='./resultados_ISd_v2',
                        help='Directorio de salida  (defecto: ./resultados_ISd_v2)')
